...
public class VerticalPager extends ViewGroup {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (OnScrollListener mListener : mListeners) {
            ...
            if (pageHeight > 0 && adjustedScrollY % pageHeight == 0) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (!mIsPagingEnabled || ev.getPointerCount() >= 2) {
            return false;
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState == TOUCH_STATE_REST) {
                    ...
                    if (view != null && (getScrollY() < 0 || getScrollY() + pageHeight > view.getBottom())) {
                        deltaY /= 2;
                    }
    ...
    private void snapToPage(final int whichPage, final int where, int duration) {
        ...
        View child = getChildAt(whichPage);
        if (child != null) {
            int delta = 0;
            if (child.getHeight() <= pageHeight || where == TOP) {
                delta = child.getTop() - getScrollY();
            } else {
                delta = child.getBottom() - pageHeight - getScrollY();
            }

            //Log.w(TAG, "SNAP TO PAGE " + whichPage);
            mScroller.startScroll(0, getScrollY(), 0, delta, duration);
            invalidate();
        } else {
            Log.w(TAG, "VERT PAGER CHILD NULL FOR CHILD " + whichPage, new Exception());
            //Toast.makeText(MainApplication.getInstance(), "Child at " + whichPage + " was NULL", Toast.LENGTH_LONG).show();
            //mScroller.startScroll(0, getScrollY(), 0, pageHeight * whichPage, duration);
            //invalidate();
        }
    }
